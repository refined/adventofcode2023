...
function getRuleOut(rules: Rule[], input: { x: number, m: number, a: number, s: number }, afterRules: string): string {
    for (const rule of rules) {
        const inputValue = input[rule.arg_name];
...
function main(input: string) {
    ...
    const inputs: { x: number, m: number, a: number, s: number }[] = [];
    for (const line of lines) {
        if (line.startsWith("{")) {
            const args = line.substring(1, line.length - 1).split(",");
            const [x, m, a, s] = args.map(a => Number(a.split("=")[1]));
            inputs.push({ x, m, a, s });
        } else {
    ...
    for (const input of inputs) {
        ...
        while (true) {
            const out = getRuleOut(rule[0], input, rule[1]);
            if (out === "R") { break; }
            if (out === "A") {
                sum += input.a + input.m + input.s + input.x;
                break;
            }
            rule = rules.get(out);
        }
