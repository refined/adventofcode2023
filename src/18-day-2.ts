...
function getOneLineSum(verticalLines: [coord, coord][], row: number) {
    ...
    for (let i = 0; i < verticalLines.length; i++) {
        ...
        if (row === lineStart[0] && row === nextLineStart[0] 
            ...
            if ((row === lineStart[0] && row === nextLineEnd[0]) 
                || (row === lineEnd[0] && row === nextLineStart[0])) { // one lines up and one line down
                linesMerged.push(linesMerged.length % 2 === 0 ? nextLineStart : lineStart);
            } else {
                if (linesMerged.length % 2 === 1) {
                    linesMerged.push(lineStart);
                    linesMerged.push(nextLineStart);
                }
            }
            i++;
        } else {
    ...
    if (linesMerged.length % 2 !== 0) {
        throw new Error("shoul not happen");
    }
...
function main(input: string) {
    ...
    for (const line of lines) {
        ...
        if (move[0] !== 0) {
            if (move[0] > 0) {
                ...
                verticalLines.push([[i + move[0] * length, j + move[1] * length], [i,j]]);
            }
        }
        i = i + move[0] * length;
        j = j + move[1] * length;
    ...
    verticalLines.sort((a, b) => a[0][1] - b[0][1]);
    verticalLines.sort((a, b) => a[0][0] - b[0][0]);

    let row = verticalLines[0][0][0];    
    let arr = verticalLines.filter(v => v[0][0] === row);
    ...
    while (arr.length) {
        ...
        if (nextRow > row + 1) {
            const oneRowSum = getOneLineSum(arr, row + 1);
            sum += oneRowSum * (nextRow - row - 1);
        }
        row = nextRow;
    }
