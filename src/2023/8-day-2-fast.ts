import { isCharNumber, getNumberLeftAndRight, Index, lcm } from '../utils'
...
function main(input: string) {
    ...
    const routMap = new Map<string, [string, string]>();   

    let steps = [];

    for (const line of lines.slice(1)) {
        const from = line.split(" = ")[0];
        const toLeft = line.split(" = ")[1].slice(1, 4);
        const toRight = line.split(" = ")[1].slice(6, 9);

        routMap.set(from, [toLeft, toRight]);
        if (from[2] === "A") {
            steps.push(from);
        }
    }
    ...
    for (let j = 0; j < steps.length; j++) {
        ...
        while (step[2] !== "Z") {
            ...
            const to = routMap.get(step);
            step = instruction === "L" ? to[0] : to[1];
