...
function isGear(ch: string | undefined): boolean {
    return ch === "*";
}

function getTwoNeighbourNumbers(row: number, col: number, lines: string[]): [number, number] | null {
    const allNumbers: Array<Index> = [];
    if (lines[row - 1]) {
        if (isCharNumber(lines[row - 1][col - 1])) allNumbers.push({ row: row - 1, col: col - 1 });
        if (isCharNumber(lines[row - 1][col])) allNumbers.push({ row: row - 1, col: col });
        if (isCharNumber(lines[row - 1][col + 1])) allNumbers.push({ row: row - 1, col: col + 1 });
    }
    if (isCharNumber(lines[row][col - 1])) allNumbers.push({ row: row, col: col - 1 });
    if (isCharNumber(lines[row][col + 1])) allNumbers.push({ row: row, col: col + 1 });
    if (lines[row + 1]) {
        if (isCharNumber(lines[row + 1][col - 1])) allNumbers.push({ row: row + 1, col: col - 1 });
        if (isCharNumber(lines[row + 1][col])) allNumbers.push({ row: row + 1, col: col });
    ...
    const all = allNumbers.map(n => getNumberLeftAndRight(lines[n.row], n.col));

    const numbers = new Set(allNumbers.map(n => getNumberLeftAndRight(lines[n.row], n.col).num));
    if (numbers.size === 2) {
        return [...numbers] as [number, number]
    }
    if (numbers.size === 1) {
        const num = [...numbers.values()][0];
        // TODO actually it could possibly be if two equal numbers are located near Gear but likelly it is not happening
        return null;
    }
...
function main(input: string) {
    ...
    const gears: Array<Index> =
        lines
            .map(
                (line, row) => [...line].map((ch, col) => isGear(ch) ? { col, row } : null)
            )
            .flat()
            .filter(t => t);

    for (const gear of gears) {
        const nums = getTwoNeighbourNumbers(gear.row, gear.col, lines);
        if (nums) {
            sum += nums[0] * nums[1];
