...
function getMore(inputValue: [number, number], compValue: number, input: XMAS | null, arg_name: string): XMAS | null {
    if (!input) return null;
    if (inputValue[0] > compValue) {
        return input;
    } else {
...
function main(input: string) {
    ...
    for (const line of lines) {
        if (line.startsWith("{")) {
            ...
            const ruleData = line.split("{")[1].split(",");
            let out = ruleData[ruleData.length - 1];
            out = out.substring(0, out.length - 1);
            const rls = ruleData.slice(0, ruleData.length - 1).map(d => ({
                outcome: d.split(":")[1],
                arg_name: d.split(":")[0].substring(0, 1), // a<2006
                comparator: d.split(":")[0].substring(1, 2) as "<" | ">",
                value: Number(d.split(":")[0].substring(2)),
            }));
            rules.set(name, [rls, out])
        }
    }
