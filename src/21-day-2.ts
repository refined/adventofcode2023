    ...
    for (const [i,j] of points) {
        for (const dir of [[0,1],[0,-1],[1,0],[-1,0]]) {
            ...
            if (0 <= nextI && nextI < rowMax && 0 <= nextJ && nextJ < colMax) {
                ...
                if (nextI < 0) outLeftPoints.push([i1, j1]);
                if (nextJ < 0) outTopPoints.push([i1, j1]);
                if (nextI >= rowMax) outRightPoints.push([i1, j1]);
                if (nextJ >= colMax) outBottomPoints.push([i1, j1]);
            }
        }
    }
...
function main(input: string, steps: number) {
    ...
    for (let _ = 0; _ < steps; _++) {
        ...
        if (_%1000 === 0 && _ > 0) {
            ...
            console.log("i:" + _ + " sum:" + sum + " cachesize:" + cache.size);
        }
    }
