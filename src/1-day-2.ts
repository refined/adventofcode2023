...
const numberFirstParts = new Set();
for (let i = 0; i < 5; i++) {
    for (const number of numbers) {
        numberFirstParts.add(number.substring(0, i + 1));
    }
}
const numberLastParts = new Set();
for (let i = 0; i < 5; i++) {
    for (const number of numbers) {
        numberLastParts.add(number.substring(number.length - i - 1, number.length).split('').reverse().join(''));
    }
}
...
function getNumber(str: string): number | null {
    switch (str) {
        case 'one': case 'eno': case '1': return 1;
        case 'two': case 'owt': case '2': return 2;
        case 'three': case 'eerht': case '3': return 3;
        case 'four': case 'ruof': case '4': return 4;
        case 'five': case 'evif': case '5': return 5;
        case 'six': case 'xis': case '6': return 6;
        case 'seven': case 'neves': case '7': return 7;
        case 'eight': case 'thgie': case '8': return 8;
        case 'nine': case 'enin': case '9': return 9;
    }
    return null;
}

function isFirstNumberPart(str: string): boolean {
    return numberFirstParts.has(str) || (str.length === 1 && str[0] >= '0' && str[0] <= '9');
}

function isLastNumberPart(str: string): boolean {
    return numberLastParts.has(str) || (str.length === 1 && str[0] >= '0' && str[0] <= '9');
}
...
function getFirstNumber(str: string): number {
    ...
    for (let i = 0; i < str.length; i++) {
        ...
        for (let j = 0; j < curr.length; j++) {
            const toTest = curr.substring(j);
            if (isFirstNumberPart(toTest)) {
                const number = getNumber(toTest);
                if (number) {
                    return number;
                }
            }
...
function getLastNumber(str: string): number {
    ...
    for (let i = str.length - 1; i >= 0; i--) {
        ...
        for (let j =  curr.length - 1; j >= 0; j--) {
            const toTest = curr.substring(curr.length - j - 1, curr.length);
            const number = getNumber(toTest);
            if (number) {
                return number;
            }
