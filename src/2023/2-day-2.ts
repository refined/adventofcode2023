...
const games = lines
  .map((line) => line.split(":"))
  .map(([first, second]) => ({
    id: Number(first.split(" ")[1]),
    rounds: second.trim().split(";"),
  }));

function getRoundCubes(round: string) {
  let cubeObj = {
    red: 0,
    green: 0,
    blue: 0,
  ...
  const cubes = round.split(",").map((t) => ({
    count: Number(t.trim().split(" ")[0].trim()),
    color: t.trim().split(" ")[1].trim(),
  }));
  for (const cube of cubes) {
    cubeObj[cube.color] = cube.count;
  }
  return cubeObj;
}
let power = 0;
...
for (const game of games) {
  ...
  for (const round of rounds) {
    const cube = getRoundCubes(round);
    max.blue = Math.max(cube.blue, max.blue);
    max.green = Math.max(cube.green, max.green);
    max.red = Math.max(cube.red, max.red);
  }

  const gameResult = max.blue * max.green * max.red;
  console.log(`id: ${id} - ${gameResult}`);
  power += gameResult;
}
