...
function isSymbol(ch: string | undefined): boolean {
    return ch !== undefined && ch !== '.' && !('0' <= ch && ch <= '9');
}

function checkNumNeighbourSymbols(num: number, row: number, colEnd: number, lines: string[]): boolean {
    const col = colEnd - num.toString().length;

    if (lines[row] && isSymbol(lines[row][col - 1]) || isSymbol(lines[row][colEnd])) {
        return true;
    }

    for (let colI = col - 1; colI < colEnd + 1; colI++) {
        if (lines[row - 1] && isSymbol(lines[row - 1][colI])
            || lines[row + 1] && isSymbol(lines[row + 1][colI])) {
            return true;
        }
    }
    return false;
    ...
    for (let row = 0; row < lines.length; row++) {
        ...
        for (let col = 0; col <= line.length; col++) {
            ...
            if ('0' <= ch && ch <= '9') {
                ...
                if (number > 0) {
                    if (checkNumNeighbourSymbols(number, row, col, lines)) {
                        sum += number;
